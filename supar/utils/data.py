...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, transform: Transform, data: Union[str, Iterable], cache: bool = False, binarize: bool = False, bin: str = None, max_len: int = None, **kwargs
    ) -> Dataset:
        ...
        if cache:
            ...
            if not self.binarize and os.path.exists(self.fbin):
                try:
                    ...
                    raise RuntimeError(f"Error found while debinarizing {self.fbin}, which may have been corrupted. " "Try re-binarizing it first!")
        else:
        ...
        if self.cache and os.path.exists(self.fbin) and not self.binarize:
            ...
            with tempfile.TemporaryDirectory() as ftemp:
                ...
                def cache(sentences):
                    ...
                    try:
                        yield ((sentences[s : s + chunk_size], fs, f"{fb}.{i}", self.max_len) for i, s in enumerate(range(0, len(sentences), chunk_size)))
                    finally:
...
class Sampler(torch.utils.data.Sampler):
    ...
    def __init__(
        self, buckets: Dict[float, List], batch_size: int, shuffle: bool = False, distributed: bool = False, even: bool = True, seed: int = 1
    ) -> Sampler:
        ...
        self.n_batches = [min(len(bucket), max(round(size * len(bucket) / batch_size), 1)) for size, bucket in zip(self.sizes, self.buckets)]
