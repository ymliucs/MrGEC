...
class Field(RawField):
    ...
    def build(self, dataset: Dataset, min_freq: int = 1, embed: Optional[Embedding] = None, norm: Callable = None) -> Field:
        r"""
        ...
        def build_vocab(dataset):
            return Vocab(
                counter=Counter(token for seq in progress_bar(getattr(dataset, self.name)) for token in self.preprocess(seq)),
                min_freq=min_freq,
...
class SubwordField(Field):
    ...
    def build(self, dataset: Dataset, min_freq: int = 1, embed: Optional[Embedding] = None, norm: Callable = None) -> SubwordField:
        if hasattr(self, 'vocab'):
            return
        ...
        def build_vocab(dataset):
            return Vocab(
                counter=Counter(piece for seq in progress_bar(getattr(dataset, self.name)) for token in seq for piece in self.preprocess(token)),
                min_freq=min_freq,
                specials=self.specials,
                unk_index=self.unk_index,
            )
    ...
    def transform(self, sequences: Iterable[List[str]]) -> Iterable[torch.Tensor]:
        for seq in sequences:
            ...
            if self.use_vocab:
                seq = [[self.vocab[i] if i in self.vocab else self.unk_index for i in token] if token else [self.unk_index] for token in seq]
            if self.bos:
                seq = [[self.bos_index]] + seq
            if self.eos:
                seq = seq + [[self.eos_index]]
...
class ChartField(Field):
    ...
    def build(self, dataset: Dataset, min_freq: int = 1) -> ChartField:
        @wait
        def build_vocab(dataset):
            return Vocab(
                counter=Counter(i for chart in progress_bar(getattr(dataset, self.name)) for row in self.preprocess(chart) for i in row if i is not None),
                min_freq=min_freq,
                specials=self.specials,
                unk_index=self.unk_index,
            )
