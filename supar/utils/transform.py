...
class Sentence(object):
    ...
    def __getstate__(self):
        state = vars(self)
        if 'fields' in state:
            state['fields'] = {name: ((value.dtype, value.tolist()) if isinstance(value, torch.Tensor) else value) for name, value in state['fields'].items()}
        return state

    def __setstate__(self, state):
        if 'fields' in state:
            state['fields'] = {
                name: (torch.tensor(value[1], dtype=value[0]) if isinstance(value, tuple) and isinstance(value[0], torch.dtype) else value)
