...
class Parser(object):
        ...
        for epoch in range(self.epoch, args.epochs + 1):
            ...
            with self.join():
                ...
                for batch in bar:
                    ...
                    self.step += 1
                    if total_steps is not None and self.cur_step >= int(total_steps):
                        end_flag = True
                        logger.info(f"Step {self.cur_step} saved")
                        break
            ...
            if self.patience < 1 or end_flag:
                break
        if is_dist():
            dist.barrier()
    ...
    def predict(
        ...
        **kwargs,
        ...
        self.transform.train() if select_min_loss else self.transform.eval()
        if args.prob:
        ...
        with tempfile.TemporaryDirectory() as t:
            ...
            for batch in progress_bar(data.loader):
                batch = self.select_step(batch) if select_min_loss else self.pred_step(batch)
